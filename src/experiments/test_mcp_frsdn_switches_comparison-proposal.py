...
def main(num_switches, num_hosts_per_switch):
    ...
    switch_list = list()
    host_list = list()
    edge_list = list() # just populate with edges between switches
    flows_list = list()
    ...
    #print host_list
    edge_list = edge_list + gen_switch_switch_links(switch_list)
...
def gen_switch_switch_links(list_of_switches):
    list_of_edges = list()
    for i in range(len(list_of_switches)):
        list_of_edges.append((list_of_switches[i-1], list_of_switches[i]))
    #Diagonal
    print len(list_of_switches)
    list_of_edges.append((list_of_switches[1], list_of_switches[1+(len(list_of_switches)/2)]))
    print list_of_edges
    return list_of_edges
...
if __name__ == "__main__":
    ...
    runtime_mcp_fd_save = open(r'runtime_mcp_switch_comparison.pickle', 'wb')
    pickle.dump(times, runtime_mcp_fd_save)
    runtime_mcp_fd_save.close()

    runtime_mcp_fd_load = open(r'runtime_mcp_switch_comparison.pickle', 'rb')
    ...
    # bx.plot(x_list, w_list, color='r', alpha=0.7, label='Packers lost in State of the Art', marker='s')
